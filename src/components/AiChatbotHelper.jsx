 ...
 * - Minimized bubble by default
 * - Auto‑expands on API errors (4xx/5xx/network)
 * - In error mode: offers analysis + chat input
 * - In normal mode: shows helpful suggestions (no typing)
 * - Draggable, resizable, smooth animations
 */
...
import { 
  ...
  BookOpen, Folder, Zap, Server, Shield, HelpCircle
} from 'lucide-react';
import clsx from 'clsx';
...
// Shared: renders a single chat message (bot or user)
const ChatMessage = ({ msg, index }) => (
  <div key={index} className={clsx("flex gap-3", msg.type === 'user' && "flex-row-reverse")}>
...
const HelpOfferScreen = ({ response, error, requestInfo, onAccept, onClose }) => (
  <div className="flex flex-col items-center justify-center h-full text-center px-4">
    ...
    <div className="w-full p-3 rounded-xl bg-red-500/5 border border-red-500/20 mb-6">
      <div className="flex items-center gap-2 text-red-400 text-xs mb-1">
        <AlertCircle className="w-3.5 h-3.5" />
        <span className="font-medium">{response?.status || 'Error'} {response?.statusText || error?.message || 'Request Failed'}</span>
      </div>
      <p className="text-xs text-gray-500 truncate">{requestInfo?.method || 'GET'} {requestInfo?.url || 'Unknown endpoint'}</p>
    </div>
    <button onClick={onAccept} className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-primary hover:bg-primary/90 text-white font-medium text-sm transition-all active:scale-95 shadow-lg shadow-primary/25">
      <Sparkles className="w-4 h-4" /> Yes, help me out!
    </button>
    <button onClick={onClose} className="mt-3 text-xs text-gray-500 hover:text-gray-400 transition-colors">No thanks, I'll figure it out</button>
  </div>
);

// Shared: chat input bar (only for error mode)
const ChatInputBar = ({ userInput, setUserInput, onSend, isProcessing }) => (
  <div className="p-3 border-t border-dark-700 bg-dark-800/50">
    <div className="flex items-center gap-2">
      <input type="text" value={userInput} onChange={(e) => setUserInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && onSend()} placeholder="Ask a follow-up question..." className="flex-1 bg-dark-900/60 border border-dark-700 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all" disabled={isProcessing} />
      <button onClick={onSend} disabled={!userInput.trim() || isProcessing} className="p-2.5 rounded-xl bg-primary hover:bg-primary/90 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95"><Send className="w-4 h-4" /></button>
    </div>
  </div>
);

// ========== NEW: SuggestionsScreen for general help ==========
const SuggestionsScreen = ({ onSelectTopic }) => {
  const topics = [
    { id: 'project', title: '📁 How to create a new project?', icon: BookOpen },
    { id: 'collection', title: '📂 How to create a collection?', icon: Folder },
    { id: 'request', title: '⚡ How to create and send a request?', icon: Zap },
    { id: 'environment', title: '🔧 How to set up environment variables?', icon: Server },
    { id: 'test', title: '✅ How to write tests?', icon: Shield },
    { id: 'error', title: '❓ Understanding API errors', icon: HelpCircle },
  ];
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        <div className="text-center mb-2">
          <div className="w-14 h-14 rounded-2xl bg-primary/20 flex items-center justify-center mx-auto mb-3"><Bot className="w-7 h-7 text-primary" /></div>
          <h4 className="text-base font-semibold text-white">How can I help you?</h4>
          <p className="text-xs text-gray-400 mt-1">Choose a topic to get started</p>
        </div>
        <div className="space-y-2">
          {topics.map(topic => (
            <button key={topic.id} onClick={() => onSelectTopic(topic.id)} className="w-full flex items-center gap-3 p-3 rounded-xl bg-dark-800/50 border border-dark-700 hover:bg-dark-700 hover:border-primary/30 transition-all text-left group">
              <topic.icon className="w-5 h-5 text-primary/70 group-hover:text-primary" />
              <span className="text-sm text-gray-200 flex-1">{topic.title}</span>
...
// ============================================================================
// MAIN COMPONENT: Always visible, draggable, minimizable chatbot
// ============================================================================
const AIChatbotHelper = ({ isVisible, onClose, error, response, requestInfo }) => {
  const [size, setSize] = useState('small');
  ...
  const [showChatInHelpMode, setShowChatInHelpMode] = useState(false); // NEW: show answer after suggestion click
  const [animatingOut, setAnimatingOut] = useState(false);
  const [animatingIn, setAnimatingIn] = useState(false);
  ...
  const dimensions = { small: { width: 320, height: 420 }, medium: { width: 420, height: 540 } };
  const currentSize = dimensions[size];
  const MARGIN = 20;

  const clampPosition = useCallback((x, y, w, h) => ({ x: Math.max(0, Math.min(x, window.innerWidth - w - MARGIN)), y: Math.max(0, Math.min(y, window.innerHeight - h - MARGIN)) }), []);

  useEffect(() => {
    const handleResize = () => !isMinimized && setPosition(prev => clampPosition(prev.x, prev.y, currentSize.width, currentSize.height));
    window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, [currentSize, isMinimized, clampPosition]);

  const handleSizeToggle = (e) => { e.stopPropagation(); const newSize = size === 'small' ? 'medium' : 'small'; const newDims = dimensions[newSize]; setSize(newSize); setPosition(prev => clampPosition(prev.x + (currentSize.width - newDims.width), prev.y, newDims.width, newDims.height)); };
  const handleMinimize = (e) => { if (e) e.stopPropagation(); if (animatingOut) return; setAnimatingOut(true); setTimeout(() => { setIsMinimized(true); setAnimatingOut(false); }, 200); };
  const handleExpand = () => { if (animatingIn) return; setIsMinimized(false); setAnimatingIn(true); setTimeout(() => setAnimatingIn(false), 50); setPosition(prev => clampPosition(prev.x, prev.y, currentSize.width, currentSize.height)); };

  const handleMouseDown = useCallback((e) => { if (e.target.closest('.chatbot-drag-handle') && !e.target.closest('button')) { setIsDragging(true); offsetRef.current = { x: e.clientX - position.x, y: e.clientY - position.y }; e.preventDefault(); } }, [position]);
  const handleMouseMove = useCallback((e) => { if (isDragging) setPosition(clampPosition(e.clientX - offsetRef.current.x, e.clientY - offsetRef.current.y, currentSize.width, currentSize.height)); }, [isDragging, currentSize, clampPosition]);
  ...
  // Auto-detect errors and switch to error mode + expand
  useEffect(() => {
    const statusCode = response?.status || error?.status || 0;
    const hasError = (statusCode >= 400 && statusCode < 600) || (error && !statusCode);
    if (hasError && !isErrorMode) {
      setIsErrorMode(true);
      setHasAcceptedHelp(false);
      setShowChatInHelpMode(false);
      setChatMessages([]);
      if (isMinimized) handleExpand();
    } else if (!hasError && isErrorMode) {
      setIsErrorMode(false);
      setHasAcceptedHelp(false);
      setShowChatInHelpMode(false);
      setChatMessages([]);
    }
  }, [error, response, isErrorMode, isMinimized, handleExpand]);

  // Reset chat when error details change (while in error mode)
  useEffect(() => {
    if (isErrorMode && (error || response)) {
      setChatMessages([]);
      setHasAcceptedHelp(false);
      setIsProcessing(false);
    }
  }, [error, response, isErrorMode]);
  ...
  const analyzeError = useCallback(async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 2000));
    const analysis = generateErrorAnalysis(response, error, requestInfo);
    setChatMessages(prev => { const updated = [...prev]; updated[updated.length-1] = { type: 'bot', content: analysis, isLoading: false }; return updated; });
    setIsProcessing(false);
  }, [error, response, requestInfo]);
  ...
  const handleSendMessage = () => {
    ...
    setIsProcessing(true);
    setTimeout(() => {
      setChatMessages(prev => [...prev, { type: 'bot', content: "I understand your concern. Based on the error analysis above, I recommend:\n\n1️⃣ Double-check your request configuration\n2️⃣ Verify all required parameters\n3️⃣ Test with a simpler request first\n\n💡 **Pro tip:** Try using the environment variables feature to manage your API keys securely.\n\nWould you like me to generate a sample request?" }]);
      setIsProcessing(false);
    }, 1500);
  };

  const handleSelectSuggestion = (topicId) => {
    const guidance = {
      project: "To create a new project:\n1. Click the 'Projects' button in the right sidebar\n2. Click 'Create Project'\n3. Enter a name and description\n4. Choose visibility (Private/Team)\n5. Click 'Create'",
      collection: "Creating a collection:\n1. Open the Collections panel\n2. Click the '+' button\n3. Select 'New Collection'\n4. Give it a name and optional description\n5. Save it to a project",
      request: "Creating and sending a request:\n1. Click 'New' in the tab bar\n2. Choose HTTP method (GET, POST, etc.)\n3. Enter the URL\n4. Add headers, body, or auth as needed\n5. Click 'Send'",
      environment: "Environment variables:\n1. Go to Variables section\n2. Create a new environment or edit an existing one\n3. Add key‑value pairs\n4. Activate the environment\n5. Use {{variableName}} in your requests",
      test: "Writing tests:\n1. Open the 'Tests' tab in the request editor\n2. Write JavaScript using `pm.test(...)`\n3. Example: `pm.test('Status is 200', () => { pm.response.to.have.status(200); });`\n4. Run the request to see test results",
      error: "Common API errors:\n• 4xx: Client errors – check your request (auth, data, URL)\n• 5xx: Server errors – the API is having issues, retry later\n• Network errors – check your internet connection or CORS settings"
    };
    const answer = guidance[topicId] || "I'll help you with that soon!";
    setChatMessages([{ type: 'user', content: `Help with: ${topicId}` }, { type: 'bot', content: answer }]);
    setShowChatInHelpMode(true);
  };
  ...
  if (isMinimized) {
    return ReactDOM.createPortal(
      <div className="fixed z-[200]" style={{ bottom: MARGIN, right: MARGIN }} data-testid="ai-chatbot-bubble-container">
        <div className="absolute inset-0 rounded-full animate-ping opacity-70 bg-primary/40" style={{ animationDuration: '1.5s' }} />
        <div className="absolute inset-0 rounded-full animate-ping opacity-40 bg-primary/20" style={{ animationDuration: '1.5s', animationDelay: '0.5s' }} />
        <button onClick={handleExpand} className="relative w-14 h-14 rounded-full shadow-2xl transition-all duration-200 hover:scale-110 bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center animate-in fade-in zoom-in">
          <Bot className="w-6 h-6 text-white" />
          {(error || (response?.status >= 400)) && <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full text-[10px] text-white flex items-center justify-center animate-pulse font-bold">!</span>}
        </button>
      </div>,
      document.body
    );
  ...
  const panelClasses = clsx("fixed z-[200] flex flex-col rounded-2xl shadow-2xl overflow-hidden bg-gradient-to-b from-dark-800 to-dark-900 border border-dark-600/50", isDragging && "cursor-grabbing select-none opacity-90", (animatingOut || animatingIn) && "transition-all duration-200 ease-out", animatingOut && "opacity-0 scale-95", animatingIn && "opacity-0 scale-95");

  return ReactDOM.createPortal(
    <div ref={dragRef} className={panelClasses} style={{ left: position.x, top: position.y, width: currentSize.width, height: currentSize.height, transition: isDragging ? 'none' : 'left 350ms cubic-bezier(0.4, 0, 0.2, 1), top 350ms cubic-bezier(0.4, 0, 0.2, 1), width 250ms ease-out, height 250ms ease-out' }} onMouseDown={handleMouseDown}>
      <div className="chatbot-drag-handle flex items-center justify-between px-4 py-3 bg-dark-800/90 border-b border-dark-700 cursor-grab active:cursor-grabbing select-none shrink-0">
        <div className="flex items-center gap-3">
          <div className="relative">
          ...
          <div><h3 className="text-sm font-semibold text-white">AI Assistant</h3><p className="text-[10px] text-gray-400">{isErrorMode ? 'Error Analysis Helper' : 'Help & Guides'}</p></div>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={handleSizeToggle} className="p-1.5 rounded-lg text-gray-400 hover:text-white hover:bg-dark-700 transition-colors">{size === 'small' ? <Maximize2 className="w-4 h-4" /> : <Minimize2 className="w-4 h-4" />}</button>
          <button onClick={handleMinimize} className="p-1.5 rounded-lg text-gray-400 hover:text-white hover:bg-dark-700 transition-colors"><X className="w-4 h-4" /></button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {isErrorMode ? (
          !hasAcceptedHelp ? (
            <HelpOfferScreen response={response} error={error} requestInfo={requestInfo} onAccept={handleAcceptHelp} onClose={handleMinimize} />
          ) : (
            <>
              {chatMessages.map((msg, idx) => <ChatMessage key={idx} msg={msg} index={idx} />)}
              {isProcessing && chatMessages.length > 0 && !chatMessages[chatMessages.length-1]?.isLoading && <TypingIndicator />}
            </>
          )
        ) : (
          // General help mode
          !showChatInHelpMode ? (
            <SuggestionsScreen onSelectTopic={handleSelectSuggestion} />
          ) : (
            <div className="space-y-4">
              <button onClick={handleBackToTopics} className="flex items-center gap-1 text-xs text-gray-400 hover:text-primary transition-colors mb-2">
      ...
      {/* Chat input only shown in error mode after accepting help */}
