...
import {
  ...
  Zap, ChevronLeft, ChevronRight, ListChecks
} from 'lucide-react';
...
export default function GenerateTestCase({ projects, activeWorkspaceId }) {
  ...
  const handleGenerateTestCases = async () => {
    ...
    setGenerating(true);
    try {
      const cases = await generateTestCases(activeSpecId);
      setTestCases(cases);
      setTestCasesTotal(cases.length);
      setTestCasesOffset(0);
      setViewMode('testcases');
      toast.success(`Generated ${cases.length} test case${cases.length !== 1 ? 's' : ''}`);
    } catch (err) {
  ...
  const handleDeleteSpec = async (e, spec) => {
    ...
    try {
      ...
      if (activeSpecId === spec.id) setActiveSpecId(specs.length > 1 ? specs[0].id : null);
