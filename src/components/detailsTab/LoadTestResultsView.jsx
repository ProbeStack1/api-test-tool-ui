...
// import { Loader2, StopCircle, ChevronDown, ChevronRight, X, Clock, Users, Gauge, Zap, Activity } from 'lucide-react';
// import clsx from 'clsx';
// import { fetchLoadTestRun, stopLoadTest } from '../../services/collectionService';
// import { toast } from 'sonner';

// // Helper components (same as before)
// function StatCard({ label, value, icon: Icon, valueClassName = "text-white" }) {
//   return (
...
// function DetailSection({ title, sectionKey, runId, expandedSections, setExpandedSections, children }) {
//   const isExpanded = expandedSections[`${runId}:${sectionKey}`];
//   const toggle = () => setExpandedSections(prev => ({
//     ...prev,
//     [`${runId}:${sectionKey}`]: !prev[`${runId}:${sectionKey}`]
//   }));

//   return (
//     <div className="border border-dark-700 rounded-lg bg-dark-800/30 overflow-hidden">
//       <div
//         className="flex items-center gap-2 px-4 py-3 bg-dark-800/30 cursor-pointer hover:bg-dark-700/30 transition-colors"
//         onClick={toggle}
//       >
...
//       </div>
//       {isExpanded && (
//         <div className="p-4 border-t border-dark-700 bg-dark-900/20">{children}</div>
//       )}
//     </div>
//   );
// }
...
//   // Log when component mounts and whenever loadTestId changes
//   useEffect(() => {
...
//   useEffect(() => {
//     if (!loadTestId) {
//       console.warn('No loadTestId provided');
//       setLoading(false);
//       setError('Invalid load test ID');
//       return;
//     }

//     console.log('Starting polling for loadTestId:', loadTestId);
//     let interval;

//     const fetchData = async () => {
//       console.log('Fetching load test run:', loadTestId);
//       try {
...
//           console.log('Test not running, stopping poll. Status:', res.data.status);
...
//     return () => {
//       console.log('Cleaning up polling for loadTestId:', loadTestId);
//       clearInterval(interval);
//     };
//   }, [loadTestId]);
...
//     console.log('Starting timer for duration:', run.config.durationSeconds);
//     const updateTimer = () => {
...
//     console.log('Stop requested for loadTestId:', loadTestId);
//     try {
...
//   if (loading) {
//     return (
//       <div className="flex-1 flex items-center justify-center bg-dark-800/80">
...
//             <h2 className="text-2xl font-bold text-white">Load Test Results</h2>
...
//             <StatCard label="Total Requests" value={result.totalRequests} />
//             <StatCard label="Avg RPS" value={result.actualRps?.toFixed(1)} />
//             <StatCard label="Avg Latency" value={`${result.avgLatencyMs?.toFixed(0)}ms`} />
//             <StatCard label="Min/Max" value={`${result.minLatencyMs}ms / ${result.maxLatencyMs}ms`} />
//             <StatCard 
//               label="Passed / Failed" 
//               value={
//                 <span>
//                   <span className="text-green-400">{result.successfulRequests}</span>
//                   <span className="text-gray-500 mx-1">/</span>
//                   <span className="text-red-400">{result.failedRequests}</span>
//                 </span>
//               }
//             />
//           </div>
...
//           {/* Tabs */}
//           <div className="border-b border-dark-700 flex gap-6">
...
//                         <div key={key} className="p-3 rounded-lg bg-dark-900/40 border border-dark-700 text-center">
//                           <p className="text-xs text-gray-500 mb-1">{key}</p>
//                           <p className="text-lg font-semibold text-white">{value}ms</p>
//                         </div>
//                       ))}
//                     </div>
//                   </div>
...
//                   <div className="rounded-xl border border-dark-700 bg-dark-800/40 p-5">
//                     <h3 className="text-sm font-medium text-white mb-4">Status Codes</h3>
//                     <div className="flex flex-wrap gap-2">
//                       {Object.entries(result.statusCodes).map(([code, count]) => (
//                         <div key={code} className="px-3 py-1.5 rounded-lg bg-dark-900/40 border border-dark-700">
...
//                 <DetailItem label="Total Requests" value={result.totalRequests} />
//                 <DetailItem label="Successful" value={result.successfulRequests} valueClassName="text-green-400" />
//                 <DetailItem label="Failed" value={result.failedRequests} valueClassName="text-red-400" />
//                 <DetailItem label="Network Errors" value={result.networkErrors || 0} />
//                 <DetailItem label="Timeouts" value={result.timeoutErrors || 0} />
//                 <DetailItem label="RPS Achieved" value={result.actualRps?.toFixed(1)} />
//                 <DetailItem label="Success RPS" value={result.successRps?.toFixed(1)} />
//                 <DetailItem label="Std Dev" value={`${result.stdDevLatencyMs?.toFixed(1)}ms`} />
//               </div>
//             )}
...
//                   <div className="rounded-xl border border-dark-700 bg-dark-800/40 p-5">
//                     <h3 className="text-sm font-medium text-white mb-4">Error Breakdown</h3>
//                     <div className="space-y-2">
//                       {Object.entries(result.errorTypes).map(([type, count]) => (
//                         <div key={type} className="flex justify-between p-2 bg-dark-900/40 rounded">
...
//       <div className="flex-1 p-8 bg-dark-800/80">
//         <div className="max-w-4xl mx-auto bg-red-500/10 border border-red-500/20 rounded-lg p-4">
//           <p className="text-red-400">Test failed: {run.errorMessage}</p>
//         </div>
//       </div>
//     );
//   }
...
//     return (
//       <div className="flex-1 p-8 bg-dark-800/80">
//         <div className="max-w-4xl mx-auto bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
//           <p className="text-yellow-400">Test stopped by user.</p>
//         </div>
//       </div>
//     );
//   }
...
// function DetailItem({ label, value, valueClassName = "text-white" }) {
//   return (
...
function StatCard({ label, value, icon: Icon, color = "text-white" }) {
  return (
    <div className="rounded-xl bg-dark-900/40 border border-dark-700 p-3 hover:border-primary/30 transition-all duration-300 group" data-testid={`stat-card-${label.toLowerCase().replace(/\s+/g, '-')}`}>
...
function SuccessRateGauge({ total, passed, failed }) {
  ...
  return (
    <div className="flex flex-col items-center" data-testid="success-gauge">
...
function LatencyDistribution({ percentiles }) {
  ...
  return (
    <div className="flex items-end gap-2 h-16" data-testid="latency-dist">
...
function PercentilesChart({ percentiles }) {
  ...
  return (
    <div className="space-y-3" data-testid="percentiles-chart">
      {entries.map(([label, value], i) => (
...
function StatusCodeChart({ statusCodes }) {
  ...
  return (
    <div className="space-y-3" data-testid="status-code-chart">
...
function RpsSparkline({ avgRps }) {
  ...
  return (
    <svg width={width} height={height} className="overflow-visible" data-testid="rps-sparkline">
      <defs>
...
export default function LoadTestResultsView({ loadTestId, onClose }) {
  ...
  if (run.status === 'completed' && result) {
    ...
    return (
      <div className="flex-1 overflow-y-auto p-8 bg-dark-800/80 backdrop-blur-sm" data-testid="results-completed">
        <div className="max-w-5xl mx-auto space-y-5">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white" data-testid="results-title">Load Test Results</h2>
          ...
          {/* Row 1: Success Gauge + Key Metrics (mirrors RunningView layout) */}
          ...
          {/* Row 2: RPS Trend + Latency Distribution (mirrors RunningView) */}
          <div className="grid grid-cols-2 gap-4">
