...
function FeatureCard({ icon: Icon, title, desc, isVisible, delay }) {
  return (
    <div
      ...
      className={`group relative p-6 rounded-2xl h-full overflow-hidden transition-all duration-500 cursor-default bg-surface border border-border hover:border-primary/40 hover:shadow-xl hover:shadow-primary/5 ${isVisible ? 'animate-slide-in-bottom' : 'opacity-0 translate-y-8'}`}
    ...
    >
      ...
      <div className="relative z-10">
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-primary/30 to-primary/10 flex items-center justify-center mb-4 text-primary group-hover:scale-110 transition-transform duration-300">
          <Icon className="w-6 h-6" />
        </div>
        <h3 className="text-lg font-semibold mb-2 text-text-primary group-hover:text-primary transition-colors duration-300">
          {title}
        </h3>
...
export default function FeaturesGrid() {
  ...
  return (
    <section data-testid="features-section" ref={ref} className="relative z-10 py-20 border-b border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex items-center gap-3 mb-10 ${isVisible ? 'animate-fade-in-left' : 'opacity-0'}`}>
          <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
            <Sparkles className="w-5 h-5 text-primary" />
          </div>
          <h2 data-testid="features-section-title" className="text-3xl font-bold text-text-primary font-heading">
            Key Features
          </h2>
        </div>
