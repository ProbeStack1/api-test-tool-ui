...
function IntegrationItem({ icon: Icon, name, color }) {
  return (
    <div className="flex-shrink-0 flex items-center gap-3 px-5 py-3 rounded-xl landing-card-bg border border-border hover:border-[#ff5b1f]/50 hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300 group cursor-default mx-2">
      <div
        className="w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-300 group-hover:scale-125 group-hover:rotate-6"
...
export default function IntegrationPartners() {
  ...
  return (
    <section data-testid="integrations-section" ref={ref} className="relative z-10 py-20 border-b border-border overflow-hidden">
      ...
      <div className={`relative mb-4 transition-all duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="flex animate-marquee" style={{ width: 'max-content', animationDuration: '20s' }}>
      ...
      <div className={`relative transition-all duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: '0.2s' }}>
        <div className="absolute left-0 top-0 bottom-0 w-24 bg-gradient-to-r from-probestack-bg to-transparent z-10" />
        <div className="absolute right-0 top-0 bottom-0 w-24 bg-gradient-to-l from-probestack-bg to-transparent z-10" />
        <div className="flex animate-marquee" style={{ width: 'max-content', animationDirection: 'reverse', animationDuration: '24s' }}>
