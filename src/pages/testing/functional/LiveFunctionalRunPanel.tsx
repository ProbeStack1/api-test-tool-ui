...
import {
  LiveFunctionalRunChart,
  type LiveStepSample,
} from "./LiveFunctionalRunChart";
...
export const LiveFunctionalRunPanel = ({ runId }: Props) => {
    ...
    const onLine = (ev: MessageEvent, kind: StreamLine["kind"]) => {
      try {
        ...
        const ok = payload.status
          ? !["FAILED", "ERROR"].includes(String(payload.status))
          : undefined;
        setLines((prev) => [
          ...prev.slice(-499),
    ...
    es.addEventListener("step.end", (e) => {
      ...
      try {
        ...
        setLatestStep({
          ...
          durationMs:
            Number(
              payload.totalMs ?? payload.durationMs ?? payload.elapsedMs ?? 0,
            ) || 0,
          ok: !["FAILED", "ERROR"].includes(
          ...
          import("@/stores/chatbot.store").then(({ useChatbot }) => {
            useChatbot.getState().triggerError({
              ...
              errorMessage:
                payload.statusReason ||
                payload.message ||
                `Run ${runId.slice(0, 8)} ended with status ${payload.status}`,
              url: `runId=${runId}`,
              method: "TEST",
            });
          });
  ...
  const progressPct =
    total > 0 ? Math.min(100, ((passed + failed + skipped) / total) * 100) : 0;
    ...
    >
      ...
      <div
        className={cn(
          ...
          !isTerminal &&
            "bg-gradient-to-r from-primary/[0.04] via-primary/[0.10] to-primary/[0.04]",
        )}
      >
        {!isTerminal && (
            ...
            {isTerminal && (
              <>
                ...
                <Button
                  ...
                  data-testid="live-start-another"
                >
      ...
      <div className="border-b border-border px-6 py-3">
        <div className="mb-1.5 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-text-muted">
          <span>Progress</span>
          <span
            data-testid="live-progress-text"
      ...
      {isTerminal && runId && (
        <div
          ...
          data-testid="endpoint-grid-section"
        >
        ...
        <div className="h-64 overflow-auto rounded-lg border border-border/60 bg-probestack-bg/40 p-3 font-mono text-[11px]">
          {lines.length === 0 ? (
            <p className="text-text-muted" data-testid="live-stream-empty">
              {isTerminal
                ? "Run completed before streaming connected. See full detail for steps."
      ...
      {run?.statusReason && (
        <div
          className="border-t border-border bg-warning/[0.06] px-6 py-3 text-xs text-warning"
          data-testid="live-status-reason"
        >
