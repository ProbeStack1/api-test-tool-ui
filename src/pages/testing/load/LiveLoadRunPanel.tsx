...
import {
  ...
  AlertOctagon, Plus, ExternalLink, Download, Gauge, Activity,
  Timer, Zap, AlertTriangle,
} from 'lucide-react';
...
interface StreamLine { ts: number; kind: string; text: string }
...
export const LiveLoadRunPanel = ({ runId }: Props) => {
  ...
  useEffect(() => {
    ...
    es.addEventListener('run.start', (e) => onEv(e as MessageEvent, 'run.start'));
  ...
  const p95        = run?.percentiles?.['95'] ?? 0;
  const p99        = run?.percentiles?.['99'] ?? 0;
  const errRate    = total > 0 ? (failed / total) * 100 : 0;

  // Approximate elapsed for progress when duration is set.
  const elapsedSec = run?.startedAt && typeof run.startedAt === 'string'
  ...
  const progressPct = isTerminal
    ? 100
    : targetDur > 0 ? Math.min(100, (elapsedSec / targetDur) * 100) : 0;

  return (
    <section data-testid="live-load-panel" className="overflow-hidden rounded-2xl border border-border bg-surface/50 shadow-sm">
      {/* status bar */}
      <div className={cn(
        'relative border-b border-border px-6 py-4',
        !isTerminal && 'bg-gradient-to-r from-amber-500/[0.04] via-amber-500/[0.10] to-amber-500/[0.04]',
      )}>
        {!isTerminal && (
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h2 className="truncate text-sm font-semibold tracking-tight" data-testid="live-load-name">
            ...
            <p className="mt-0.5 text-[11px] text-text-muted">
              ...
              {run?.config?.targetRps ? <> · target {run.config.targetRps} rps</> : <> · unlimited</>}
              {run?.config?.durationSeconds ? <> · {run.config.durationSeconds}s</> : null}
              {run?.config?.rampUpSeconds ? <> · ramp {run.config.rampUpSeconds}s</> : null}
            </p>
          </div>
          <div className="ml-auto flex items-center gap-1.5">
            {isRunning && (
              <Button size="sm" variant="outline" onClick={() => pauseMut.mutate()} disabled={pauseMut.isPending} data-testid="live-load-pause">
                {pauseMut.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Pause className="h-3.5 w-3.5" />} Pause
              </Button>
            )}
            {isPaused && (
              <Button size="sm" variant="outline" onClick={() => resumeMut.mutate()} disabled={resumeMut.isPending} data-testid="live-load-resume">
                {resumeMut.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />} Resume
              </Button>
            )}
            {(isRunning || isPaused) && (
              <Button size="sm" variant="ghost" onClick={() => cancelMut.mutate()} disabled={cancelMut.isPending} data-testid="live-load-cancel">
            ...
            {isTerminal && (
              <>
                ...
                <Button size="sm" variant="outline" onClick={() => openLoadRun(runId)} data-testid="live-load-open-detail">
                  <ExternalLink className="h-3.5 w-3.5" /> Open detail
                </Button>
                <Button size="sm" variant="primary" onClick={() => setLiveRun(null)} data-testid="live-load-start-another">
      ...
      <div className="grid gap-3 border-b border-border bg-elevated/20 px-6 py-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        <Tile icon={Activity} label="Requests" value={total.toLocaleString()} testId="live-load-kpi-total" />
        <Tile icon={Zap}      label="RPS"       value={actualRps.toFixed(1)} tone="amber" testId="live-load-kpi-rps" />
        <Tile icon={Timer}    label="Avg latency" value={`${avgLat.toFixed(0)}ms`} testId="live-load-kpi-avg" />
        <Tile icon={Gauge}    label="p95"       value={`${p95}ms`} testId="live-load-kpi-p95" />
        <Tile icon={Gauge}    label="p99"       value={`${p99}ms`} testId="live-load-kpi-p99" />
        <Tile icon={CheckCircle2} label="Success"  value={successful.toLocaleString()} tone="success" testId="live-load-kpi-success" />
        <Tile icon={XCircle}      label="Errors"   value={`${failed.toLocaleString()} (${errRate.toFixed(2)}%)`} tone="danger" testId="live-load-kpi-errors" />
      </div>
      ...
      <div className="border-b border-border px-6 py-3">
        <div className="mb-1.5 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-text-muted">
          <span>Progress</span>
          <span data-testid="live-load-progress" className="font-mono normal-case text-text-secondary">
            {targetDur > 0 ? <>{Math.min(elapsedSec, targetDur).toFixed(0)}s / {targetDur}s</> : <>{elapsedSec.toFixed(0)}s elapsed</>}
        ...
        <div className="h-2 overflow-hidden rounded-full bg-elevated">
          {targetDur > 0
            ? <div className={cn('h-full transition-all duration-500',
                isTerminal ? (failed > 0 ? 'bg-danger' : 'bg-success') : 'bg-amber-400',
      ...
      <div className="px-6 py-4" data-testid="live-load-stream">
        <div className="mb-2 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-text-muted">
          <span>Activity</span>
          {!isTerminal && !streamConnected && <span className="text-warning">· stream reconnecting…</span>}
        </div>
        <div className="h-56 overflow-auto rounded-lg border border-border/60 bg-probestack-bg/40 p-3 font-mono text-[11px]">
          {lines.length === 0
            ? <p className="text-text-muted" data-testid="live-load-stream-empty">
                {isTerminal ? 'Run completed before streaming connected.' : 'Waiting for first tick…'}
              </p>
            : lines.map((l, i) => (
                <div key={i} className="flex gap-2 leading-5">
                  <span className="w-16 shrink-0 text-text-muted">{new Date(l.ts).toLocaleTimeString()}</span>
                  <span className="w-20 shrink-0 font-semibold text-text-secondary">{l.kind}</span>
      ...
      {!!run?.thresholdViolations?.length && (
        <div className="border-t border-border bg-danger/[0.06] px-6 py-3 text-xs text-danger" data-testid="live-load-violations">
...
const Tile = ({ icon: Icon, label, value, tone = 'default', testId }: {
  icon: any; label: string; value: number | string; tone?: 'default' | 'success' | 'danger' | 'amber'; testId: string;
}) => {
