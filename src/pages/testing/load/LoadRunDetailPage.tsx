...
export const LoadRunDetailPage = () => {
  ...
  const invalidate = () =>
    qc.invalidateQueries({ queryKey: ["loadTest", "run", id] });
  const pauseMut = useMutation({
    mutationFn: () => pauseRun(id!),
    onSuccess: invalidate,
    onError: (e: any) => setActionError(e?.message ?? "pause failed"),
  });
  const resumeMut = useMutation({
    mutationFn: () => resumeRun(id!),
    onSuccess: invalidate,
    onError: (e: any) => setActionError(e?.message ?? "resume failed"),
  });
  const cancelMut = useMutation({
    mutationFn: () => cancelRun(id!),
    onSuccess: invalidate,
    onError: (e: any) => setActionError(e?.message ?? "cancel failed"),
  });
  ...
  return (
    <div className="flex h-full flex-col" data-testid="load-run-detail-page">
      <header className="border-b border-border bg-surface/30 px-6 pt-3">
        <div className="flex items-center gap-2">
          <Button
            size="sm"
            variant="ghost"
            onClick={() => closeLoadRun()}
            data-testid="load-run-back"
          >
          ...
          {run?.passed === true && (
            <span className="rounded border border-success/30 bg-success/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-success">
              PASSED
            </span>
          )}
          {run?.passed === false && (
            <span className="rounded border border-danger/30 bg-danger/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-danger">
              FAILED THRESHOLDS
            </span>
          )}
        ...
        {run && (
          <div className="mt-1.5 flex flex-wrap items-center gap-3 text-[10px] text-text-muted">
            ...
            {run.testSpecId && (
              <span>
                · spec{" "}
                <span className="font-mono">{run.testSpecId.slice(0, 8)}</span>
              </span>
            )}
            {run.config?.concurrency != null && (
              <span>· concurrency {run.config.concurrency}</span>
            )}
            {run.config?.targetRps != null && run.config.targetRps > 0 && (
              <span>· target {run.config.targetRps} rps</span>
            )}
            {run.config?.durationSeconds != null && (
              <span>· {run.config.durationSeconds}s</span>
            )}
            {run.config?.rampUpSeconds != null && (
              <span>· ramp {run.config.rampUpSeconds}s</span>
            )}
            {run.queuedAt && (
              <span>
                · queued{" "}
                {formatRelative(
                  typeof run.queuedAt === "string" ? run.queuedAt : "",
                )}
              </span>
            )}
          </div>
      ...
      {actionError && (
        <div className="mx-6 mt-3 rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-xs text-danger">
          {actionError}
        </div>
      )}
      ...
      <div className="flex-1 overflow-auto p-6">
        {!run || runQ.isLoading ? (
          ...
          <>
            <section
              data-testid="load-run-kpi-grid"
              className="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7"
            >
              <KPI
                label="Total req"
                value={(run.totalRequests ?? 0).toLocaleString()}
                icon={Activity}
              />
              <KPI
                label="Success"
                value={(run.successfulRequests ?? 0).toLocaleString()}
                tone="success"
                icon={Activity}
              />
              <KPI
                label="Failed"
                value={(run.failedRequests ?? 0).toLocaleString()}
                tone="danger"
                icon={ZapOff}
              />
              <KPI
                label="Actual RPS"
                value={(run.actualRps ?? 0).toFixed(1)}
                icon={Gauge}
              />
              <KPI
                label="Avg latency"
                value={`${(run.avgLatencyMs ?? 0).toFixed(0)}ms`}
                icon={Timer}
              />
              <KPI
                label="p95"
                value={`${run.percentiles?.["p95"] ?? run.percentiles?.["95"] ?? 0}ms`}
                tone="warning"
                icon={Timer}
              />
              <KPI
                label="p99"
                value={`${run.percentiles?.["p99"] ?? run.percentiles?.["99"] ?? 0}ms`}
                tone="danger"
                icon={Timer}
              />
            </section>
            ...
            {run.thresholdViolations && run.thresholdViolations.length > 0 && (
              <section
                data-testid="load-run-violations"
                className="mb-3 rounded-md border border-danger/30 bg-danger/10 px-4 py-3 text-xs text-danger"
              >
            ...
            <section
              ...
              data-testid="load-run-endpoints-section"
            >
              ...
              {!run.endpointResults || run.endpointResults.length === 0 ? (
                <p
                  ...
                  data-testid="load-run-endpoints-empty"
                >
                  No per-endpoint data yet.
                </p>
              ) : (
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {run.endpointResults.map((e, i) => (
                      <tr
                        key={`${e.endpointName ?? i}`}
                        className="border-t border-border/40"
                      >
                        <td className="px-3 py-1.5 truncate font-medium">
                          {e.endpointName ?? e.url ?? "—"}
                        </td>
                        <td className="px-3 py-1.5 font-mono text-[10px] uppercase">
                          {e.method ?? "—"}
                        </td>
                        <td className="px-3 py-1.5 text-right font-mono text-[10px]">
                          {(e.totalRequests ?? 0).toLocaleString()}
                        </td>
                        <td className="px-3 py-1.5 text-right font-mono text-[10px] text-success">
                          {(e.successfulRequests ?? 0).toLocaleString()}
                        </td>
                        <td className="px-3 py-1.5 text-right font-mono text-[10px] text-danger">
                          {(e.failedRequests ?? 0).toLocaleString()}
                        </td>
                        <td className="px-3 py-1.5 text-right font-mono text-[10px]">
                          {(e.avgLatencyMs ?? 0).toFixed(0)}
                        </td>
                        <td className="px-3 py-1.5 text-right font-mono text-[10px]">
                          {e.p95Ms ?? 0}
                        </td>
                        <td className="px-3 py-1.5 text-right font-mono text-[10px]">
                          {(e.errorRatePct ?? 0).toFixed(2)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
const KPI = ({
  ...
  return (
    <div className="rounded-lg border border-border bg-surface/40 p-3">
      ...
      <div className={cn("text-lg font-semibold tracking-tight", tones[tone])}>
        {value}
      </div>
    </div>
  );
