...
const TABS = [
  ...
  { key: "runs", label: "Runs", icon: ListTree, testId: "load-tab-runs" },
  {
    ...
    testId: "load-tab-dashboard",
  },
  {
    key: "schedules",
    label: "Schedules",
    icon: CalendarClock,
    testId: "load-tab-schedules",
  },
  {
    key: "analytics",
    label: "Analytics",
    icon: LineChart,
    testId: "load-tab-analytics",
  },
] as const;
...
export const LoadTestsPage = () => {
  ...
  const trackedCount = useRunsTracker(
    (s) => Object.values(s.runs).filter((r) => r.kind === "load").length,
  );

  if (!ws) {
    return (
      <NoProjectEmpty
        testId="load-no-workspace"
        icon="load-test"
        surface="load tests"
      />
    );
      ...
      <header className="border-b border-border bg-surface/30 px-6 pt-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-text-muted">
            · Concurrency · target RPS · ramp-up · thresholds. Throughput &amp;
            p95 from the Java engine.
          </span>
          {trackedCount > 0 && (
            <span
              data-testid="load-tracked-count"
...
const LoadRecentRuns = ({ workspaceId }: { workspaceId: string }) => {
  ...
  const cancelMut = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["loadTest"] }),
  });
  const pauseMut = useMutation({
    mutationFn: (id: string) => pauseRun(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["loadTest"] }),
  });
  const resumeMut = useMutation({
    mutationFn: (id: string) => resumeRun(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["loadTest"] }),
  });
  ...
  const toggleSelect = (run: LoadRun) => {
    setSelected((prev) => {
      if (prev.some((r) => r.runId === run.runId))
        return prev.filter((r) => r.runId !== run.runId);
  ...
  return (
    <section
      className="overflow-hidden rounded-2xl border border-border bg-surface/40 shadow-sm"
      data-testid="load-recent-section"
    >
      <header className="flex flex-wrap items-center gap-3 border-b border-border bg-elevated/30 px-5 py-3">
        <h2 className="flex items-center gap-2 text-sm font-semibold tracking-tight">
          ...
          <span className="rounded-full bg-elevated px-2 py-0.5 font-mono text-[10px] text-text-secondary">
            {q.data?.totalElements ?? 0}
          </span>
        </h2>
        {selected.length > 0 && (
          <span
            className="text-[10px] text-text-muted"
            data-testid="load-compare-count"
          >
        ...
        >
          ...
          {selected.length === 2 && "2 runs"}
        </Button>
        <select
          data-testid="load-runs-filter"
          value={status}
          onChange={(e) => {
      ...
      {q.isLoading ? (
        ...
        <div
          ...
          data-testid="load-runs-list-empty"
        >
              ...
              {runs.map((r) => {
                ...
                return (
                  <tr
                    key={r.runId}
                    className={cn(
                      "border-t border-border/40 hover:bg-hover/40",
                      checked && "bg-primary/[0.06]",
                    )}
                    data-testid={`load-runs-row-${r.runId}`}
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <button
                        onClick={() => openLoadRun(r.runId)}
                        className="font-medium hover:text-primary"
                      >
                        {r.name || r.runId.slice(0, 8)}
                      </button>
                    </td>
                    <td className="px-3 py-2.5">
                    ...
                    <td className="px-3 py-2.5 font-mono text-[10px]">
                      {(r.actualRps ?? 0).toFixed(1)}
                    </td>
                    <td className="px-3 py-2.5 font-mono text-[10px]">
                      {r.percentiles?.["p95"] ?? r.percentiles?.["95"] ?? 0}ms
                    </td>
                    <td className="px-3 py-2.5 font-mono text-[10px] text-danger">
                      {r.failedRequests ?? 0}
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-right">
                      {r.status === "RUNNING" && (
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => pauseMut.mutate(r.runId)}
                        >
                      ...
                      {r.status === "PAUSED" && (
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => resumeMut.mutate(r.runId)}
                        >
                      ...
                      {(r.status === "RUNNING" || r.status === "PAUSED") && (
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => cancelMut.mutate(r.runId)}
                        >
...
const LoadDashboard = ({ workspaceId }: { workspaceId: string }) => {
  ...
  return (
    <div className="h-full overflow-auto" data-testid="load-dashboard">
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-6">
        <section
          data-testid="load-kpi-grid"
          className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7"
        >
          <Tile
            icon={Activity}
            label="Total runs"
            value={dash?.runsTotal ?? 0}
            testId="load-kpi-total"
          />
          <Tile
            icon={PlayCircle}
            label="Running"
            value={dash?.runsRunning ?? 0}
            tone="amber"
            testId="load-kpi-running"
          />
          <Tile
            icon={CheckCircle2}
            label="Success"
            value={dash?.runsSuccess ?? 0}
            tone="success"
            testId="load-kpi-success"
          />
          <Tile
            icon={XCircle}
            label="Failed"
            value={dash?.runsFailed ?? 0}
            tone="warning"
            testId="load-kpi-failed"
          />
          <Tile
            icon={AlertOctagon}
            label="Error"
            value={dash?.runsError ?? 0}
            tone="danger"
            testId="load-kpi-error"
          />
          <Tile
            icon={Ban}
            label="Cancelled"
            value={dash?.runsCancelled ?? 0}
            tone="muted"
            testId="load-kpi-cancelled"
          />
          <Tile
            icon={Gauge}
            label="Pass rate"
            value={`${(dash?.passRatePct ?? 0).toFixed(1)}%`}
            tone="success"
            testId="load-kpi-passrate"
          />
        </section>

        <section
          className="flex flex-wrap items-center gap-3 rounded-2xl border border-amber-500/30 bg-gradient-to-r from-amber-500/[0.06] via-amber-500/[0.10] to-amber-500/[0.06] px-5 py-4 shadow-sm"
          data-testid="load-cta-card"
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-text-muted">
              Pick a source, set concurrency &amp; duration, expand{" "}
              <em>Advanced</em> for ramp-up.
            </p>
          </div>
          <Button
            variant="primary"
            ...
            className="ml-auto"
            data-testid="load-dashboard-go-runs"
          >
        ...
        <section className="overflow-hidden rounded-2xl border border-border bg-surface/40 shadow-sm">
          <header className="flex items-center justify-between gap-2 border-b border-border bg-elevated/30 px-5 py-3">
            ...
            <Button
              ...
              variant="ghost"
              onClick={() => setTab("runs")}
              data-testid="load-dashboard-see-all"
            >
          ...
          {runsQ.isLoading ? (
            ...
            <div className="p-10 text-center" data-testid="load-runs-empty">
              ...
              <p className="mt-1 text-xs text-text-muted">
                Head to the <strong>Runner</strong> tab to queue one.
              </p>
            </div>
          ) : (
            <ul
              className="divide-y divide-border"
              data-testid="load-recent-runs"
            >
              {runs.map((r) => (
                <LoadRunRow
                  key={r.runId}
...
const Tile = ({
  ...
  return (
    <div
      data-testid={testId}
      className="rounded-xl border border-border bg-surface/40 p-4 transition-colors hover:bg-surface/60"
    >
      <div className="mb-1.5 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-text-muted">
        <Icon className="h-3 w-3" /> {label}
      </div>
      <div className={cn("text-xl font-semibold tracking-tight", tones[tone])}>
        {value}
      </div>
    </div>
  );
...
const LoadRunRow = ({
  ...
  return (
    <li
      data-testid={`load-run-row-${run.runId}`}
    ...
    >
      ...
      <span className="font-mono text-[10px] text-text-muted">
        {(run.actualRps ?? 0).toFixed(1)} rps
      </span>
      <span className="font-mono text-[10px] text-text-muted">
        p95 {run.percentiles?.["p95"] ?? run.percentiles?.["95"] ?? 0}ms
      </span>
      ...
      <span className="flex shrink-0 items-center gap-0.5">
        {isRunning && (
          <Button
            ...
            data-testid={`load-pause-${run.runId}`}
          >
            <Pause className="h-3.5 w-3.5" />
          </Button>
        )}
        {isPaused && (
          <Button
            ...
            data-testid={`load-resume-${run.runId}`}
          >
            <Play className="h-3.5 w-3.5" />
          </Button>
        )}
        {(isRunning || isPaused) && (
          <Button
            ...
            data-testid={`load-cancel-${run.runId}`}
          >
            <Ban className="h-3.5 w-3.5" />
          </Button>
        )}
...
const LoadSchedulesList = ({ workspaceId }: { workspaceId: string }) => {
  ...
  const q = useQuery({
    queryKey: ["loadTest", "schedules", workspaceId],
    queryFn: () => listSchedules(workspaceId, { size: 50 }),
  });
  const invalidate = () =>
    qc.invalidateQueries({ queryKey: ["loadTest", "schedules", workspaceId] });
  const pauseMut = useMutation({
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-3 flex items-center gap-2">
          ...
          <Button
            ...
            data-testid="load-schedule-create-btn"
            className="ml-auto"
          >
        ...
        <div className="overflow-auto rounded-lg border border-border">
          {q.isLoading ? (
            ...
            <div
              ...
              data-testid="load-schedules-empty"
            >
            ...
            <ul className="divide-y divide-border">
              {items.map((s) => (
                <li
                  ...
                  data-testid={`load-schedule-row-${s.scheduleId}`}
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="truncate font-medium">
                        {s.name || s.scheduleId.slice(0, 8)}
                      </span>
                      <span
                        className={cn(
                    ...
                    <div className="mt-0.5 flex flex-wrap items-center gap-2 text-[10px] text-text-muted">
                      ...
                      {s.lastRunStatus && (
                        <span>· last: {s.lastRunStatus}</span>
                      )}
                      {s.nextRunAt && (
                        <span>
                          · next{" "}
                          {formatRelative(
                            typeof s.nextRunAt === "string" ? s.nextRunAt : "",
                          )}
                        </span>
                      )}
                    </div>
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => triggerMut.mutate(s.scheduleId)}
                  ...
                  {s.status === "PAUSED" ? (
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => resumeMut.mutate(s.scheduleId)}
                    >
                      <Play className="h-3.5 w-3.5" />
                    </Button>
                  ) : (
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => pauseMut.mutate(s.scheduleId)}
                    >
                      <Pause className="h-3.5 w-3.5" />
                    </Button>
                  )}
        ...
        <ConfirmDialog
          ...
          description={
            deleteTarget
              ? `"${deleteTarget.name ?? deleteTarget.scheduleId}" will be permanently deleted.`
              : ""
          }
          confirmText="Delete"
          tone="danger"
          onConfirm={async () => {
            if (deleteTarget)
              await deleteMut.mutateAsync(deleteTarget.scheduleId);
          }}
        />
      </div>
    </div>
  ...
  const mut = useMutation({
    mutationFn: () =>
      createSchedule({
        ...
        collectionId:
          source === "collection" && collectionId ? collectionId : undefined,
      ...
      <div className="space-y-3">
        <Field label="Name">
          <input
            ...
            className={cls()}
            placeholder="Hourly load smoke"
          />
        </Field>
        <Field label="Run source" required>
          <div
            className="flex gap-2 text-[11px]"
            data-testid="load-schedule-source"
          >
            <label
              className={`flex items-center gap-1.5 rounded border px-2 py-1 cursor-pointer ${source === "spec" ? "border-primary bg-primary/10 text-primary" : "border-border"}`}
            >
              <input
                type="radio"
                className="hidden"
                checked={source === "spec"}
                onChange={() => setSource("spec")}
              />
              Test Spec
            </label>
            <label
              className={`flex items-center gap-1.5 rounded border px-2 py-1 cursor-pointer ${source === "collection" ? "border-primary bg-primary/10 text-primary" : "border-border"}`}
            >
              <input
                type="radio"
                className="hidden"
        ...
        {source === "spec" && (
          <Field label="Test spec" required>
            <select
              ...
              className={cls()}
              data-testid="load-schedule-spec"
            >
        ...
        {source === "collection" && (
          <Field label="Collection" required>
            <select
              ...
              className={cls()}
              data-testid="load-schedule-collection"
            >
              <option value="">— select —</option>
        ...
        <Field label="Environment">
          ...
          >
            ...
            {(envsQ.data ?? []).map((e: any) => {
              ...
              return (
                <option
                  key={e.id ?? e.environmentId}
                  value={e.id ?? e.environmentId}
                >
                  {e.name} ({scope})
                </option>
              );
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Schedule" required>
            <select
              value={
                LOAD_CRON_PRESETS.some((p) => p.value === cron)
                  ? cron
                  : "__custom__"
              }
              onChange={(e) => {
          ...
          <Field label="Timezone">
            <select
              ...
              className={cls()}
              data-testid="load-schedule-tz"
            >
              {LOAD_TZ_PRESETS.map((tz) => (
...
const LoadAnalytics = ({ workspaceId }: { workspaceId: string }) => {
  ...
  if (!testSpecId && specs[0])
    setTimeout(() => setTestSpecId(specs[0].testSpecId), 0);
      ...
      <div className="mx-auto max-w-7xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <select
            value={testSpecId}
            onChange={(e) => setTestSpecId(e.target.value)}
            data-testid="load-analytics-spec"
          ...
          <select
            ...
            data-testid="load-analytics-limit"
            className="h-7 rounded border border-border bg-probestack-bg px-2 text-xs"
          >
            {[10, 20, 30, 50].map((n) => (
  ...
  if (points.length === 0)
    return (
      <p
        ...
        data-testid="load-trend-empty"
      >
        Not enough load runs yet.
      </p>
    );
      ...
      <div className="mb-4 flex h-20 items-end gap-1">
        {points.map((p) => {
          ...
          return (
            <div
              key={p.runId}
              title={`${p.status} · ${p.actualRps} rps · p95 ${p.p95Ms}ms · ${p.errorRatePct}% errors`}
              className={cn(
          ...
          {[...points]
            ...
            .map((p) => (
              <tr key={p.runId} className="border-t border-border/40">
                <td className="px-3 py-1.5 text-[10px] text-text-muted">
                  {formatRelative(typeof p.at === "string" ? p.at : "")}
                </td>
                <td className="px-3 py-1.5">
                  <RunStatusBadge status={String(p.status)} />
                </td>
                <td className="px-3 py-1.5 font-mono text-[10px]">
                  {(p.actualRps ?? 0).toFixed(1)}
                </td>
                <td className="px-3 py-1.5 font-mono text-[10px]">
                  {p.p95Ms ?? 0}ms
                </td>
...
/* shared form bits */
const cls = () =>
  "h-8 w-full rounded border border-border bg-probestack-bg px-2 text-xs";
const Field = ({
  label,
  children,
  required,
